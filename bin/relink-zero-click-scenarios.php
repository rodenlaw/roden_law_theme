<?php
/**
 * Repoint editorial links that point at the 64 zero-click practice-area
 * scenario pages retired 2026-09-25 (docs/scenario-pages-plan-2026-09-25.md).
 *
 * Run BEFORE bin/remove-zero-click-scenarios.php, which refuses to apply while any
 ...
 * targets are read from roden_zero_click_scenario_urls() in inc/legacy-redirects.php.
 * These are child practice_area pages, so the NESTED form
 * (/practice-areas/{pillar}/{scenario}/) is rewritten as well as the flat one.
 *
 *   Dry run (default):
 *     ssh $H "wp --path=$P eval-file -" < bin/relink-zero-click-scenarios.php \
 *       > docs/backups/zero-click-scenario-relink-$(date +%Y-%m-%d).json
 *
 *   Apply:
 *     ssh $H "wp --path=$P eval-file - apply" < bin/relink-zero-click-scenarios.php \
 *       > docs/backups/zero-click-scenario-relink-$(date +%Y-%m-%d).json
 */

$apply = isset( $args[0] ) && 'apply' === $args[0];
$err   = fopen( 'php://stderr', 'w' );
fprintf( $err, "%s — relink the 64 zero-click scenario pages\n\n", $apply ? 'APPLY' : 'DRY RUN' );

if ( ! function_exists( 'roden_zero_click_scenario_urls' ) ) {
    fprintf( $err, "ABORT: roden_zero_click_scenario_urls() is not defined — deploy the theme change first.\n" );
    exit( 1 );
}
$map = roden_zero_click_scenario_urls();

/* Nested alias: /x-lawyers/scenario/ is also /practice-areas/x-lawyers/scenario/. */
$aliases = array();
foreach ( $map as $path => $target ) {
    if ( preg_match( '#^/([a-z0-9-]+-lawyers)/([a-z0-9-]+)/$#', $path, $m ) ) {
        $aliases[ '/practice-areas/' . $m[1] . '/' . $m[2] . '/' ] = $target;
    }
}
$all = $map + $aliases;
...
$backup = array(
    'generated' => gmdate( 'c' ),
    'batch'     => 'zero-click-scenario-relink',
...
if ( $apply ) {
    fprintf( $err, "Now re-run remove-zero-click-scenarios.php dry run — it should report no link debt.\n" );
}

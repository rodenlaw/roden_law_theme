<?php
/**
 * Seeder: bicycle-accident-lawyers × Charleston, SC intersection page (ENRICH)
 *
 * UPDATES the existing templated stub /bicycle-accident-lawyers/charleston-sc/ in place
 ...
 *
 * Usage:
 *   wp eval-file wp-content/themes/roden-law/inc/seed-bicycle-charleston.php
 * Safe to re-run: updates the existing page in place (does NOT create a new one).
 */

defined( 'ABSPATH' ) || exit;

WP_CLI::log( 'Enriching bicycle-accident-lawyers × Charleston, SC...' );
...
<h2>How Charleston Bicycle Crashes Happen</h2>
<p>Charleston's dense peninsula streets and heavy tourist and student cycling create constant conflict with vehicles:</p>
<ul>
<li><strong>Right-hook and left-hook crashes</strong> — drivers turning across a cyclist's path at intersections and driveways.</li>
<li><strong>Dooring</strong> — parked-car occupants opening doors into the path of a passing cyclist on downtown streets.</li>
<li><strong>Unsafe passing</strong> — motorists squeezing past cyclists without the required three feet of clearance.</li>
<li><strong>Failure to yield</strong> — drivers ignoring a cyclist's right of way at crossings and on shared corridors.</li>
</ul>
...
$postarr = array(
	'post_type'    => 'practice_area',
	'post_status'  => 'publish',
	'post_title'   => 'Bicycle Accident Lawyers in Charleston, SC',
...
$faqs = array(
	array(
		'question' => 'Who are the best bicycle accident lawyers in Charleston, SC?',
	...
	array(
		...
		'answer'   => 'The general deadline is three years from the date of injury under S.C. Code § 15-3-530. Bicycle injuries are often severe, so it is best to have your case reviewed as soon as possible while evidence and witnesses are available.',
	),
	array(
		'question' => 'How much does a Charleston bicycle accident lawyer cost?',
		'answer'   => 'Roden Law handles Charleston bicycle accident cases on a contingency fee basis. You pay no attorney fees and no upfront costs, and we are only paid if we win. If there is no recovery, you owe us nothing.',
	),
);
update_post_meta( $post_id, '_roden_faqs', $faqs );

WP_CLI::success( "Enriched /bicycle-accident-lawyers/charleston-sc/ — post ID {$post_id}" );

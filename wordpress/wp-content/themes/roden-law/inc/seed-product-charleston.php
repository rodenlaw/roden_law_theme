<?php
/**
 * Seeder: product-liability-lawyers × Charleston, SC intersection page (ENRICH)
 *
 * UPDATES the existing thin templated stub /product-liability-lawyers/charleston-sc/
 ...
 *
 * Usage:
 *   wp eval-file wp-content/themes/roden-law/inc/seed-product-charleston.php
 * Safe to re-run: updates the existing page in place (does NOT create a new one).
 */

defined( 'ABSPATH' ) || exit;

WP_CLI::log( 'Enriching product-liability-lawyers × Charleston, SC...' );
...
<h2>Why Choose Roden Law for a Charleston Product Liability Claim</h2>
<p>Product cases are among the most technical injury claims — they require engineering analysis, preservation of the defective product, and manufacturers with deep-pocketed national defense counsel. What separates Roden Law is direct attorney involvement paired with the engineering and safety experts needed to prove the defect. Our office at 127 King Street, Suite 200 sits downtown minutes from the Charleston County Circuit Court, so investigation and filings happen without delay.</p>
<ul>
...
<h2>Charleston Product Liability Cases We Handle</h2>
<p>Defective-product injuries in the Lowcountry span consumer goods, vehicles, and industrial equipment:</p>
<ul>
<li><strong>Auto and tire defects</strong> — defective airbags, tire failures, and vehicle component defects tied to crashes on I-526 and area highways.</li>
...
<li><strong>Industrial and port equipment</strong> — machinery, lifts, and equipment failures at Charleston's industrial and maritime sites.</li>
...
$postarr = array(
	'post_type'    => 'practice_area',
	'post_status'  => 'publish',
	'post_title'   => 'Product Liability Lawyers in Charleston, SC',
...
$faqs = array(
	array(
		'question' => 'Who are the best product liability lawyers in Charleston, SC?',
		'answer'   => 'Roden Law is a leading product liability firm serving Charleston and the Lowcountry, with a 4.9-star average from hundreds of client reviews and more than $300 million recovered for injured clients across Georgia and South Carolina. Our Charleston office at 127 King Street handles every case on a contingency fee basis — no fee unless we win — and retains the engineering experts needed to prove a defect. Call (843) 790-8999 for a free consultation.',
	),
	array(
	...
	array(
		'question' => 'How much does a Charleston product liability lawyer cost?',
		'answer'   => 'Roden Law handles Charleston product liability cases on a contingency fee basis. You pay no attorney fees and no upfront costs — including the cost of the engineering experts needed to build your case — and we are only paid if we win. If there is no recovery, you owe us nothing.',
	),
);
update_post_meta( $post_id, '_roden_faqs', $faqs );

WP_CLI::success( "Enriched /product-liability-lawyers/charleston-sc/ — post ID {$post_id}" );

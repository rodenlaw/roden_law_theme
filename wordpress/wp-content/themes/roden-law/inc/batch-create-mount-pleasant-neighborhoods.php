<?php
/**
 * Batch Create Mount Pleasant Sub-Neighborhood Pages
 *
 * Run via WP-CLI:
 *   wp eval-file wp-content/themes/roden-law/inc/batch-create-mount-pleasant-neighborhoods.php
 *
 * Creates location CPT posts as children of the Mount Pleasant neighborhood page.
...
if ( ! defined( 'ABSPATH' ) || ! class_exists( 'WP_CLI' ) ) {
    echo "This script must be run via WP-CLI: wp eval-file wp-content/themes/roden-law/inc/batch-create-mount-pleasant-neighborhoods.php\n";
    exit( 1 );
}

$data_file = get_template_directory() . '/inc/data/mount-pleasant-neighborhoods.json';
...
$parent = get_page_by_path( 'south-carolina/charleston/mount-pleasant', OBJECT, 'location' );
if ( ! $parent ) {
    WP_CLI::error( 'Mount Pleasant location page not found at path: south-carolina/charleston/mount-pleasant' );
    return;
}

WP_CLI::log( sprintf( 'Mount Pleasant parent page found: ID %d', $parent->ID ) );
...
foreach ( $neighborhoods as $n ) {
    $existing = get_page_by_path( 'south-carolina/charleston/mount-pleasant/' . $n['slug'], OBJECT, 'location' );
    if ( $existing ) {

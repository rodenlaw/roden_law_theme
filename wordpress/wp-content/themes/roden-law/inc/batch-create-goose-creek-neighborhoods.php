<?php
/**
 * Batch Create Goose Creek Sub-Neighborhood Pages
 *
 * Run via WP-CLI:
 *   wp eval-file wp-content/themes/roden-law/inc/batch-create-goose-creek-neighborhoods.php
 *
 * Creates location CPT posts as children of the Goose Creek neighborhood page.
...
if ( ! defined( 'ABSPATH' ) || ! class_exists( 'WP_CLI' ) ) {
    echo "This script must be run via WP-CLI: wp eval-file wp-content/themes/roden-law/inc/batch-create-goose-creek-neighborhoods.php\n";
    exit( 1 );
}

$data_file = get_template_directory() . '/inc/data/goose-creek-neighborhoods.json';
...
$parent = get_page_by_path( 'south-carolina/charleston/goose-creek', OBJECT, 'location' );
if ( ! $parent ) {
    WP_CLI::error( 'Goose Creek location page not found at path: south-carolina/charleston/goose-creek' );
    return;
}

WP_CLI::log( sprintf( 'Goose Creek parent page found: ID %d', $parent->ID ) );
...
foreach ( $neighborhoods as $n ) {
    $existing = get_page_by_path( 'south-carolina/charleston/goose-creek/' . $n['slug'], OBJECT, 'location' );
    if ( $existing ) {

<?php
/**
 * Batch Create Effingham County Sub-Neighborhood Pages
 *
 * Run via WP-CLI:
 *   wp eval-file wp-content/themes/roden-law/inc/batch-create-effingham-county-neighborhoods.php
 *
 * @package RodenLaw
 */

if ( ! defined( 'ABSPATH' ) || ! class_exists( 'WP_CLI' ) ) {
    echo "This script must be run via WP-CLI: wp eval-file wp-content/themes/roden-law/inc/batch-create-effingham-county-neighborhoods.php\n";
    exit( 1 );
}

$data_file = get_template_directory() . '/inc/data/effingham-county-neighborhoods.json';
...
$parent = get_page_by_path( 'georgia/savannah/effingham-county', OBJECT, 'location' );
if ( ! $parent ) {
    WP_CLI::error( 'Effingham County location page not found at path: georgia/savannah/effingham-county' );
    return;
}

WP_CLI::log( sprintf( 'Effingham County parent page found: ID %d', $parent->ID ) );
...
foreach ( $neighborhoods as $n ) {
    $existing = get_page_by_path( 'georgia/savannah/effingham-county/' . $n['slug'], OBJECT, 'location' );
    if ( $existing ) {

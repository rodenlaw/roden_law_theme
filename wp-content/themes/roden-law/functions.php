...
// them to unrelated posts or add trailing slashes that break rewrite rules.
...
// WP Engine mangles /wp-sitemap.xml (no trailing slash) into sitemap=wp
// instead of sitemap=index. Detect this and redirect to the working URL.
...
// Intercept the /es/ sitemap XML request and return 404 so crawlers stop fetching it.
// The /es/ sub-index entry is generated by the translation plugin; to fully remove it
// from the sitemap index, disable sitemaps in the translation plugin settings (WPML →
// SEO Options → Sitemaps, or Polylang → Settings → URL Modifications).
add_action( 'template_redirect', 'roden_block_es_sitemap', 0 );
function roden_block_es_sitemap() {
    if ( isset( $_SERVER['REQUEST_URI'] ) && preg_match( '#^/es/wp-sitemap#', $_SERVER['REQUEST_URI'] ) ) {
        status_header( 404 );
        nocache_headers();
        echo '<!-- Spanish sitemap disabled -->';
        exit;
    }
}
...
function roden_exclude_toxic_pages_from_sitemap( $args, $post_type ) {
    if ( 'page' === $post_type ) {
        $exclude_slugs = array(
            ...
            'savannah',
            'charleston',
            // 'class-action-lawyers' removed — pillar page is now live and should be indexed
            'savannah-ppi-attorney',                                // PPC landing page.
            'free-consultation-with-charleston-personal-injury-lawyer', // PPC landing page.
            'south-carolina-truck-accident-lawyer',                 // Duplicates PA CPT intersection.
...
function roden_register_page_templates( $templates ) {
    ...
    $templates['templates/template-landing-ga-car-accident.php'] = 'Landing Page — GA Car Accident';
    return $templates;
}

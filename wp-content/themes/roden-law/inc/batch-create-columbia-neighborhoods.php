<?php
/**
 * Batch Create Columbia Neighborhood Pages
 *
 * Run via WP-CLI:
 *   wp eval-file wp-content/themes/roden-law/inc/batch-create-columbia-neighborhoods.php
 *
 * Reads from inc/data/columbia-neighborhoods.json
 * Creates location CPT posts as children of the Columbia location page.
...
if ( ! defined( 'ABSPATH' ) || ! class_exists( 'WP_CLI' ) ) {
    echo "This script must be run via WP-CLI: wp eval-file wp-content/themes/roden-law/inc/batch-create-columbia-neighborhoods.php\n";
    exit( 1 );
}

// Load neighborhood data
$data_file = get_template_directory() . '/inc/data/columbia-neighborhoods.json';
...
// Get Columbia parent post ID
$parent = get_page_by_path( 'south-carolina/columbia', OBJECT, 'location' );
if ( ! $parent ) {
    WP_CLI::error( 'Columbia location page not found at path: south-carolina/columbia' );
    return;
}

WP_CLI::log( sprintf( 'Columbia parent page found: ID %d', $parent->ID ) );
...
foreach ( $neighborhoods as $n ) {
    ...
    $existing = get_page_by_path(
        'south-carolina/columbia/' . $n['slug'],
        OBJECT,
        'location'
    );
    ...
    update_post_meta( $post_id, '_roden_parent_office_key', 'columbia' );

<?php
/**
 * Batch Create West Ashley Sub-Neighborhood Pages
 *
 * Run via WP-CLI:
 *   wp eval-file wp-content/themes/roden-law/inc/batch-create-west-ashley-neighborhoods.php
 *
 * @package RodenLaw
 */

if ( ! defined( 'ABSPATH' ) || ! class_exists( 'WP_CLI' ) ) {
    echo "This script must be run via WP-CLI: wp eval-file wp-content/themes/roden-law/inc/batch-create-west-ashley-neighborhoods.php\n";
    exit( 1 );
}

$data_file = get_template_directory() . '/inc/data/west-ashley-neighborhoods.json';
...
$parent = get_page_by_path( 'south-carolina/charleston/west-ashley', OBJECT, 'location' );
if ( ! $parent ) {
    WP_CLI::error( 'West Ashley location page not found at path: south-carolina/charleston/west-ashley' );
    return;
}

WP_CLI::log( sprintf( 'West Ashley parent page found: ID %d', $parent->ID ) );
...
foreach ( $neighborhoods as $n ) {
    $existing = get_page_by_path( 'south-carolina/charleston/west-ashley/' . $n['slug'], OBJECT, 'location' );
    if ( $existing ) {

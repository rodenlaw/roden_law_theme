<?php
/**
 * Batch Create Charleston Neighborhood Pages
 *
 * Run via WP-CLI:
 *   wp eval-file wp-content/themes/roden-law/inc/batch-create-neighborhoods.php
 *
 * Reads from inc/data/charleston-neighborhoods.json
 * Creates location CPT posts as children of the Charleston location page.
...
if ( ! defined( 'ABSPATH' ) || ! class_exists( 'WP_CLI' ) ) {
    echo "This script must be run via WP-CLI: wp eval-file inc/batch-create-neighborhoods.php\n";
    exit( 1 );
}

// Load neighborhood data
$data_file = get_template_directory() . '/inc/data/charleston-neighborhoods.json';
...
// Get Charleston parent post ID
$charleston = get_page_by_path( 'south-carolina/charleston', OBJECT, 'location' );
if ( ! $charleston ) {
    WP_CLI::error( 'Charleston location page not found at path: south-carolina/charleston' );
    return;
}

WP_CLI::log( sprintf( 'Charleston parent page found: ID %d', $charleston->ID ) );
...
foreach ( $neighborhoods as $n ) {
    ...
    $existing = get_page_by_path(
        'south-carolina/charleston/' . $n['slug'],
        OBJECT,
        'location'
    );

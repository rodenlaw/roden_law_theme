<?php
/**
 * Batch Create Downtown Charleston Sub-Neighborhood Pages
 *
 * Run via WP-CLI:
 *   wp eval-file wp-content/themes/roden-law/inc/batch-create-downtown-charleston-neighborhoods.php
 *
 * Creates location CPT posts as children of the Downtown Charleston neighborhood page.
...
if ( ! defined( 'ABSPATH' ) || ! class_exists( 'WP_CLI' ) ) {
    echo "This script must be run via WP-CLI: wp eval-file wp-content/themes/roden-law/inc/batch-create-downtown-charleston-neighborhoods.php\n";
    exit( 1 );
}

$data_file = get_template_directory() . '/inc/data/downtown-charleston-neighborhoods.json';
...
$parent = get_page_by_path( 'south-carolina/charleston/downtown-charleston', OBJECT, 'location' );
if ( ! $parent ) {
    WP_CLI::error( 'Downtown Charleston location page not found at path: south-carolina/charleston/downtown-charleston' );
    return;
}

WP_CLI::log( sprintf( 'Downtown Charleston parent page found: ID %d', $parent->ID ) );
...
foreach ( $neighborhoods as $n ) {
    $existing = get_page_by_path( 'south-carolina/charleston/downtown-charleston/' . $n['slug'], OBJECT, 'location' );
    if ( $existing ) {

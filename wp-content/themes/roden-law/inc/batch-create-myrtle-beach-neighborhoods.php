<?php
/**
 * Batch Create Myrtle Beach Neighborhood Pages
 *
 * Run via WP-CLI:
 *   wp eval-file wp-content/themes/roden-law/inc/batch-create-myrtle-beach-neighborhoods.php
 *
 * Reads from inc/data/myrtle-beach-neighborhoods.json
 * Creates location CPT posts as children of the Myrtle Beach location page.
...
if ( ! defined( 'ABSPATH' ) || ! class_exists( 'WP_CLI' ) ) {
    echo "This script must be run via WP-CLI: wp eval-file wp-content/themes/roden-law/inc/batch-create-myrtle-beach-neighborhoods.php\n";
    exit( 1 );
}

// Load neighborhood data
$data_file = get_template_directory() . '/inc/data/myrtle-beach-neighborhoods.json';
...
// Get Myrtle Beach parent post ID
$parent = get_page_by_path( 'south-carolina/myrtle-beach', OBJECT, 'location' );
if ( ! $parent ) {
    WP_CLI::error( 'Myrtle Beach location page not found at path: south-carolina/myrtle-beach' );
    return;
}

WP_CLI::log( sprintf( 'Myrtle Beach parent page found: ID %d', $parent->ID ) );
...
foreach ( $neighborhoods as $n ) {
    ...
    $existing = get_page_by_path(
        'south-carolina/myrtle-beach/' . $n['slug'],
        OBJECT,
        'location'
    );

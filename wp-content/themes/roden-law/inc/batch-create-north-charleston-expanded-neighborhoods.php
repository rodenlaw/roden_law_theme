<?php
/**
 * Batch Create North Charleston Expanded Neighborhood Pages
 *
 * Run via WP-CLI:
 *   wp eval-file wp-content/themes/roden-law/inc/batch-create-north-charleston-expanded-neighborhoods.php
 *
 * Reads from inc/data/north-charleston-expanded-neighborhoods.json
 * Creates location CPT posts as children of the North Charleston location page.
...
if ( ! defined( 'ABSPATH' ) || ! class_exists( 'WP_CLI' ) ) {
    echo "This script must be run via WP-CLI: wp eval-file wp-content/themes/roden-law/inc/batch-create-north-charleston-expanded-neighborhoods.php\n";
    exit( 1 );
}

// Load neighborhood data
$data_file = get_template_directory() . '/inc/data/north-charleston-expanded-neighborhoods.json';
...
WP_CLI::log( sprintf( 'North Charleston parent page found: ID %d', $north_charleston->ID ) );
...
foreach ( $neighborhoods as $n ) {
    ...
    $post_id = wp_insert_post( array(
        ...
        'post_title'   => $n['name'],
        'post_name'    => $n['slug'],
        'post_parent'  => $north_charleston->ID,
        'post_status'  => 'draft',
    ...
    update_post_meta( $post_id, '_roden_parent_office_key', 'north-charleston' );

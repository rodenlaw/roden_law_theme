<?php
/**
 * Batch Create Savannah Neighborhood Pages
 *
 * Run via WP-CLI:
 *   wp eval-file wp-content/themes/roden-law/inc/batch-create-savannah-neighborhoods.php
 *
 * Reads from inc/data/savannah-neighborhoods.json
 * Creates location CPT posts as children of the Savannah location page.
...
if ( ! defined( 'ABSPATH' ) || ! class_exists( 'WP_CLI' ) ) {
    echo "This script must be run via WP-CLI: wp eval-file wp-content/themes/roden-law/inc/batch-create-savannah-neighborhoods.php\n";
    exit( 1 );
}

// Load neighborhood data
$data_file = get_template_directory() . '/inc/data/savannah-neighborhoods.json';
...
// Get Savannah parent post ID
$savannah = get_page_by_path( 'georgia/savannah', OBJECT, 'location' );
if ( ! $savannah ) {
    WP_CLI::error( 'Savannah location page not found at path: georgia/savannah' );
    return;
}

WP_CLI::log( sprintf( 'Savannah parent page found: ID %d', $savannah->ID ) );
...
foreach ( $neighborhoods as $n ) {
    ...
    // Build post content from content_hook
    $post_content = '';
    ...
    // Set neighborhood meta fields
    update_post_meta( $post_id, '_roden_is_neighborhood', true );
    update_post_meta( $post_id, '_roden_parent_office_key', 'savannah' );
    ...
    // Store H1 override
    if ( ! empty( $n['h1'] ) ) {
        update_post_meta( $post_id, '_roden_neighborhood_h1', $n['h1'] );
    }

    // Store priority tier
    if ( ! empty( $n['priority'] ) ) {
        update_post_meta( $post_id, '_roden_neighborhood_priority', $n['priority'] );
    }

    // Store FAQs in the standard _roden_faqs format
    if ( ! empty( $n['faqs'] ) && is_array( $n['faqs'] ) ) {
        update_post_meta( $post_id, '_roden_faqs', $n['faqs'] );
    }

    // SEO meta (for Yoast or Rank Math)
...
WP_CLI::success( sprintf(
    ...
    $created, $skipped, $errors, count( $neighborhoods )
) );

if ( $created > 0 ) {
    WP_CLI::log( '' );
    WP_CLI::log( 'All new pages are saved as DRAFTS. Review them in the WordPress admin,' );
    WP_CLI::log( 'then publish when ready. Remember to flush permalinks after publishing:' );
